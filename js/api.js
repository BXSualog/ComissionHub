...
const API = {
  ...
  async request(path, method = 'GET', data = null) {
    ...
    try {
      ...
      return {
        success: false,
        message: 'Could not connect to the server. Please ensure the Node server is running on port 3000.'
      };
    }
  },
  ...
  async signup(firstName, lastName, email, password) {
    return this.request('/api/auth/signup', 'POST', { firstName, lastName, email, password });
  },
  ...
  async checkEmail(email) {
    return this.request('/api/auth/check-email', 'POST', { email });
  },
