...
async function initClientPage() {
  ...
  // If user is already logged in and visits the landing page, redirect to dashboard
  ...
  $('#btn-do-login')?.addEventListener('click', async () => {
    ...
    closeModal();
    const userName = res.user?.name || 'there';
...
function initAdminPage() {
  ...
  async function renderAnalyticsContent() {
    ...
    const revenueRes = await API.getRevenueStats();
    const pendingRes = await API.getPendingSummary();

    if (summaryRes.success) {
      const s = summaryRes.data;
      $('#ana-avg-value').textContent = `₱${Math.round(s.avg_order_value).toLocaleString()}`;
      $('#ana-top-service').textContent = s.top_service || 'None';
      $('#ana-active-clients').textContent = s.active_clients || 0;
    }

    const revTbody = $('#analytics-revenue-tbody');
    if (revTbody && revenueRes.success) {
      const data = revenueRes.data;
      
      revTbody.innerHTML = data.map(r => {
        const share = Math.round(r.share_percentage || 0);
        return `
          <tr>
            <td data-label="Service Type">${escapeHtml(r.service_type)}</td>
            <td data-label="Orders">${r.order_count}</td>
            <td data-label="Total Revenue">₱${parseFloat(r.total_revenue).toLocaleString()}</td>
            <td data-label="Share">
    ...
    const penTbody = $('#analytics-pending-tbody');
    if (penTbody && pendingRes.success) {
      const data = pendingRes.data;
      if (data.length === 0) {
        penTbody.innerHTML = '<tr><td colspan="2"><div class="empty-state">No pending orders.</div></td></tr>';
      } else {
        penTbody.innerHTML = data.map(p => `
          <tr>
            <td data-label="Client Name">${escapeHtml(p.name)}</td>
            <td data-label="Pending Requests">
              <span class="badge badge-pending">${p.pending_count}</span>
            </td>
          </tr>
        `).join('');
      }
